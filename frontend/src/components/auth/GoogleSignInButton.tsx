"use client";

import React, { useEffect, useRef, useState } from "react";
import { useAuth } from "@/context/AuthContext";
import { env } from "@/config/env";
import { Loader2 } from "lucide-react";
import { toast } from "sonner";

declare global {
  interface Window {
    google?: any;
  }
}

export const GoogleSignInButton: React.FC = () => {
  const { googleLogin } = useAuth();
  const [loading, setLoading] = useState(false);
  const [gsiReady, setGsiReady] = useState(false);
  const containerRef = useRef<HTMLDivElement>(null);

  const clientId =
    env.googleClientId ||
    "604011563193-ft5ril7p9cv01jtaldutqn5gplvpadn2.apps.googleusercontent.com";

  useEffect(() => {
    if (typeof window === "undefined") return;

    let isMounted = true;

    const renderGoogleButton = () => {
      if (!isMounted) return;
      if (window.google?.accounts?.id && containerRef.current) {
        try {
          window.google.accounts.id.initialize({
            client_id: clientId,
            callback: async (credentialResponse: any) => {
              if (credentialResponse?.credential) {
                if (isMounted) setLoading(true);
                try {
                  await googleLogin(credentialResponse.credential);
                } catch (err: any) {
                  console.error("Google Auth error:", err);
                  toast.error("Google Sign-In failed", {
                    description: err?.message || "Authentication error. Please try again.",
                  });
                } finally {
                  if (isMounted) setLoading(false);
                }
              } else {
                toast.error("Google Sign-In was cancelled or failed");
              }
            },
            auto_select: false,
            cancel_on_tap_outside: true,
            use_fedcm_for_button: true,
          });

          // Render Google's native official button (Zero redirect_uri needed!)
          if (containerRef.current) {
            containerRef.current.innerHTML = "";
            window.google.accounts.id.renderButton(containerRef.current, {
              theme: "filled_black",
              size: "large",
              type: "standard",
              shape: "rectangular",
              text: "continue_with",
              width: "360",
              logo_alignment: "left",
            });
          }

          if (isMounted) setGsiReady(true);

          try {
            window.google.accounts.id.prompt();
          } catch {
            // Prompt display is optional
          }
        } catch (e) {
          console.error("Error rendering Google Sign-In button:", e);
        }
      }
    };

    if (window.google?.accounts?.id) {
      renderGoogleButton();
    } else {
      const scriptId = "google-gsi-client-script";
      let script = document.getElementById(scriptId) as HTMLScriptElement | null;
      if (!script) {
        script = document.createElement("script");
        script.id = scriptId;
        script.src = "https://accounts.google.com/gsi/client";
        script.async = true;
        script.defer = true;
        script.onload = () => {
          renderGoogleButton();
        };
        document.body.appendChild(script);
      } else {
        script.addEventListener("load", () => {
          renderGoogleButton();
        });
      }
    }

    return () => {
      isMounted = false;
    };
  }, [clientId, googleLogin]);

  return (
    <div className="w-full flex flex-col items-center justify-center min-h-[44px]">
      {loading ? (
        <div className="w-full max-w-[360px] h-11 flex items-center justify-center px-4 rounded-xl border border-slate-700 bg-slate-800/80 text-slate-200 text-sm font-medium shadow-sm">
          <Loader2 className="w-4 h-4 animate-spin text-emerald-400 mr-2.5" />
          <span>Signing you in with Google...</span>
        </div>
      ) : (
        <>
          <div
            ref={containerRef}
            className={`w-full flex justify-center transition-opacity duration-200 ${
              gsiReady ? "opacity-100 min-h-[44px]" : "opacity-0 h-0 pointer-events-none"
            }`}
          />

          {!gsiReady && (
            <div className="w-full max-w-[360px] h-11 flex items-center justify-center gap-3 px-4 rounded-xl border border-slate-800 bg-slate-900/60 text-slate-300 text-sm font-medium shadow-sm animate-pulse">
              <svg className="w-4 h-4 flex-shrink-0" viewBox="0 0 24 24">
                <path
                  fill="#4285F4"
                  d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                />
                <path
                  fill="#34A853"
                  d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                />
                <path
                  fill="#FBBC05"
                  d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"
                />
                <path
                  fill="#EA4335"
                  d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z"
                />
              </svg>
              <span className="text-xs font-semibold">Loading Google Sign-In...</span>
            </div>
          )}
        </>
      )}
    </div>
  );
};
