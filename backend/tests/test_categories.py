...
async def test_category_lifecycle():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        ...
        res = await ac.get("/api/v1/categories", headers=HEADERS)
        assert res.status_code == 200
        data = res.json()["data"]
        assert len(data) >= 6

        # 2. Create custom category
        create_res = await ac.post("/api/v1/categories", json={"name": "Travel Tech"}, headers=HEADERS)
        ...
        # 3. Rename category
        update_res = await ac.put(f"/api/v1/categories/{cat_id}", json={"name": "Gadgets"}, headers=HEADERS)
